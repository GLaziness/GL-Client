...
public class TraceDialog extends BaseDialog{
    ...
    public void show(Player player, TraceInfo info, boolean offline){
        ...
        buttons.button(arc.Core.bundle.get("gl.ui.trace.1"), () -> ui.showConfirm("@confirm", Core.bundle.format("confirmban", player.name()), () ->
            Call.sendChatMessage("/js Vars.netServer.admins.banPlayerID(" + info.uuid + "); Vars.netServer.admins.banPlayerIP(" + info.ip + ")")
        )).width(420);
        if(!offline) buttons.button(arc.Core.bundle.get("gl.ui.trace.2"), () -> ui.showConfirm("@confirm", Core.bundle.format("confirmban", player.name()), () -> Call.adminRequest(player, Packets.AdminAction.ban, null))).width(420);
